...
int main(int argc, char ** argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		if (argv[i][0] == '-')
		{
            ...
            else if (string(argv[i]) == "-I" && i + 1 < argc) img_filename = argv[++i];
            else if (string(argv[i]) == "-O" && i + 1 < argc) output_filename = argv[++i];
            else if (string(argv[i]) == "-S" && i + 1 < argc) stipple_file = argv[++i];
            else if (string(argv[i]) == "-weighted_color") hedcut.use_weighted_color = true;
            else if (string(argv[i]) == "-area_radius") hedcut.use_area_radius = true;
			else
    ...
    if(!stipple_file.empty()) {

        cv::Mat stipple_image = cv::imread(img_filename.c_str(), CV_LOAD_IMAGE_COLOR);   // Read the file
        if (!stipple_image.data) {
            cout << "! Error: Could not open or find the stipple image" << std::endl;
            return -1;
        }
        if (hedcut.build(image, stipple_image) == false)
