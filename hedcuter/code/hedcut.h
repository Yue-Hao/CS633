...
public:
    ...
    bool build(cv::Mat & input_image, cv::Mat & stipple_image);
    ...
    bool use_weighted_color;
    bool use_area_radius;

private:

	void sample_initial_points(cv::Mat & img, int n, std::vector<cv::Point2d> & pts);
    int sample_initial_points(cv::Mat & img, std::vector<cv::Point2d> & pts);
	void create_disks(cv::Mat & img, CVT & cvt);
    void create_disks_weighted_color(cv::Mat & img, CVT & cvt);
    void create_disks_area_radius(cv::Mat & img, CVT & cvt);
